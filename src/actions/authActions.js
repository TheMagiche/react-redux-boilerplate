...
// Set token when logged in so that you can get data from the server
    ...
    (dispatch) => {
        const promise = new Promise((resolve, reject) => {
            instance.post('auth/signup', data)
                .then((res) => {
                    dispatch({ type: 'SET_REGISTER', value: 'Thank you for registering' });
                    resolve(res);
                }, (err) => {
                    reject(err);
                })
